...
const fs = require('fs');
const url = 'https://en.wikipedia.org/wiki/Xi%27an_Jiaotong-Liverpool_University';
const jsonPath = "./json/"; 
...
//take soupTag in the function
function getParagraphText(soupTag){
    let paragraphs = soupTag.findAll('p');//array 
    let text = '';//array
    for(let i = 0; i < paragraphs.length; i++){
        let p = paragraphs[i].getText().toLowerCase();

        if(p.indexOf("2+2 programmes") != -1){
            console.log(p);
            text += p;
          }
